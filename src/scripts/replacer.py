...
for ciid in ciid_list:
    ...
    # build path to asset archive file
    data_path = f'/Volumes/Asset_Archive/{brand}Web/content/{padded_ciid[:4]}/{padded_ciid[4:7]}/{padded_ciid[7:]}/cn{ciid}.html'
    # build path at which to backup current file - filename is time/date/user/brand/ciid stamped
    backup_path = f'Backups/{sysuser}:{datetime.today().strftime("%m-%d-%y-%H:%M:%S")}--{brand}-cn{ciid}.html'
    # add file path to list for push to repo
...
# commit and push all backups to repo - you will have file backed up locally and in the repo
commit_message = f'Add {len(file_list)} backup files - replacing "{replace_this}" with "{with_this}" by {sysuser} at {datetime.today().strftime("%m-%d-%H:%M")}'
...
for ciid in ciid_list:
    ...
    path = f'/Volumes/Asset_Archive/{brand}Web/content/{padded_ciid[:4]}/{padded_ciid[4:7]}/{padded_ciid[7:]}/cn{ciid}.html'
...
print(f'{bcolors.WARNING}Switching back to master branch')
git.checkout('master')
